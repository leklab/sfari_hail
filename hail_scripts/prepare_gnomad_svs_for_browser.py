...
p.add_argument("--vcf", help="URL of gnomAD SV VCF", required=True)
p.add_argument("--output", help="URL to write Hail table to", required=True)
args = p.parse_args()

hl.init(log="/tmp/hail.log")

ds = hl.import_vcf(args.vcf, force_bgz=True).rows()
...
ds = ds.annotate(
    ...
    pos=ds.info.POS_hg38,
    xpos=xpos(ds.locus.contig, ds.info.POS_hg38),
    end_chrom=ds.info.CHR2,
    end_pos=ds.info.END_hg38,
    end_xpos=xpos(ds.info.CHR2, ds.info.END_hg38),
